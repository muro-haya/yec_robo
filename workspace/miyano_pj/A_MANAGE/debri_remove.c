...
//左右切り替えの適合地も必要か　左右切り替えた場合、角度とライントレースだけ反対にすればよい（*-1か）
...
bool_t cyc_debri_remove( void ){
    ...
    case 1:  //ボトル迄直進
        ...
        break;    
    case 2:  //ボトル色判定（青ならcase+1, 赤ならcase=0）
        dr_jdg_color();
        break;
    */
    case 3:  //デブリボトル運搬（直進）
    ...
    case 4:  //デブリボトル運搬（後退）
        ...
        break;
    case 5:  //ボトルカウント＆判定（デブリボトル2以上ならcase+1, デブリボトル2未満ならcase=0）
        dr_jdg_bottle_count();
        break;
    case 6:  //攻略終了地点に向けて回転
...
void dr_jdg_color(){
    ...
    else if( BLUE == color_result ){  //青ならフェイズを+1
        g_u16_debri_remove_phase += 1;
    }
}
*/
...
/* ボトルカウント＆判定フェイズ */
void dr_jdg_bottle_count(){
    g_u16_debri_count += 1;  //デブリボトルのカウントを+1
